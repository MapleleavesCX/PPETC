...
// sm2的签名函数--------------------------------------------------
void sm2_sign(vector<string>& sign, string& message, string& sk) {
...
// sm2的验签函数--------------------------------------------------
bool sm2_verify(string& message, vector<string>& sign, vector<string>& pk) {
...
bool sm2_dec(string& M, string& sk, vector<string>& c1c2c3) {
    ...
    BIGNUM* big_u = BN_new();
    BIGNUM* C3 = BN_new();
    BN_hex2bn(&big_u, U.c_str());
    BN_hex2bn(&C3, c1c2c3[2].c_str());
    
    if (BN_cmp(big_u, C3) != 0)
    {
        cout << "U != C3\n";
        //释放内存
        BN_free(big_u);
